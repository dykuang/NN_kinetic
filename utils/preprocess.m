...
function [data,aT,ar] = preprocess(file_name)
...
%% compute rate of conversion degree with respect to time
% using central difference.
data(1,7)=(data(2,6)-data(1,6))/(data(2,1)-data(1,1));
data(end,7)= (data(end,6)-data(end-1,6))/(data(end,1)-data(end-1,1)); 
data(2:end-1,7) = (data(3:end,6)-data(1:end-2,6))./(data(3:end,1)-data(1:end-2,1));

DC_smooth = smooth(data(:,1),data(:,7),0.015,'rloess');
